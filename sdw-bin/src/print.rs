    ...
    for node in block {
        match &**node {
            Node::Function {
                ...
                print_idn(
                    format!("a function with name '{}' which returns '{}'", name, type_disp(return_ty)),
                    idn,
                );
                for (idx, pm) in params.iter().enumerate() {
                    print_idn(format!("- parameter {}: '{}', of type '{}'", idx, pm.0, type_disp(&pm.1)), idn + 1);
                }
                print(body, idn + 1);
            }
            Node::Return { expr } => {
                ...
                if let Some(expr) = expr {
                    print_idn(format!("with return expression evaluating to '{}'", expr.eval()), idn + 1);
                }
            }
            Node::VDec { name, init } => {
